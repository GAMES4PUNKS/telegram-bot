...
# Patch event loop for environments like Render
nest_asyncio.apply()
...
async def link_wallet(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in pending_challenges:
        del pending_challenges[user_id]
    await update.message.reply_text(f"🔗 Wallet {wallet} linked & verified! Now use /plaE or /spacerun to play.")
...
async def welcome_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    welcome_message = (
        ...
        f"You can play our featured games and earn NFTs:\n"
        f"▶️ /plaE – Play Emojis Invade\n"
        f"🚀 /spacerun – Play Spacerun3008\n"
        f"⚡ /status – Check game status\n\n"
        f"Start by verifying with /lfg and linking your WAX wallet with /linkEwallet.\n\n"
