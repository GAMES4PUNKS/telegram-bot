...
# Memory store for math challenges
pending_challenges = {}
...
async def start(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(f"🧠 Solve: What is {a} + {b}?")
...
async def handle_message(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if user_id in pending_challenges:
        ...
        await update.message.reply_text("❌ Use /start to verify.")

# --- Wallet Link Commands ---
...
async def verify_ekey(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data["data"]:
        ...
        await update.message.reply_text(f"🚫 NFT missing.\nBuy here:\n{GAME_KEY_DROP_URL}")

# --- Game Launch Commands ---
async def plaE(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await _launch_game(update, REQUIRED_TEMPLATE_ID, EMOJIS_INVADE_URL)

async def spacerun(update: Update, context: ContextTypes.DEFAULT_TYPE):
    await _launch_game(update, REQUIRED_TEMPLATE_ID, SPACERUN_URL)

async def _launch_game(update, template_id, game_url):
    user_id = update.effective_user.id
    async with aiosqlite.connect("botdata.db") as db:
        async with db.execute("SELECT wallet FROM linked_wallets WHERE telegram_id = ?", (user_id,)) as cursor:
            result = await cursor.fetchone()
    if not result:
        ...
        return
    wallet = result[0]
    url = f"https://wax.api.atomicassets.io/atomicassets/v1/assets?owner={wallet}&template_id={template_id}&collection_name=games4punks1"
    async with aiohttp.ClientSession() as session:
        async with session.get(url) as resp:
            data = await resp.json()
    if data["data"]:
...
async def welcome_new_member(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    await update.message.reply_text(
        ...
        f"🗝 NFT Needed: GK3008 Game Key\n"
        f"🔗 Buy: {GAME_KEY_DROP_URL}\n"
        f"📲 Link: /linkEwallet\n"
        f"🧠 Verify: /start"
    )

# --- FastAPI Status Routes ---
@api.get("/")
async def root():
